...
function [INLIERSp1X, INLIERSp1Y, INLIERSp2X, INLIERSp2Y] = RANSAC(N_max, thresh, match1X, match1Y, match2X, match2Y)
    ...
    H = est_homography(match1X,match1Y, match2X,match2Y);
    ...
    while (iter < N_max && (percentage < 0.50))
        ...
        [Hp2X, Hp2Y] = apply_homography(H, match2X, match2Y);
        Hp2 = [Hp2X, Hp2Y];            
        ...
        for i=1:size(ssds, 1)
            ...
            if (ssd < thresh)
                ...
                INLIERSXY = [INLIERSXY; [match1X(i), match1Y(i), match2X(i), match2Y(i)]];
